...
class HBM3CustomMap final : public HBM3LinearMapperBase, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IAddrMapper, HBM3CustomMap, "HBM3-Custom", "Applies a HBM3 custom mapping to the address. (Ro Ba BG Co Pch Ch)");
  ...
  public:
    ...
    void apply(Request& req) override {
      ...
      req.addr_vec[1] = slice_lower_bits(addr, m_addr_bits[1]); // Pseudochannel
      req.addr_vec[5] = slice_lower_bits(addr, m_addr_bits[5]); // Column
      req.addr_vec[2] = slice_lower_bits(addr, m_addr_bits[2]); // Bankgroup
      req.addr_vec[3] = slice_lower_bits(addr, m_addr_bits[3]); // Bank
      req.addr_vec[4] = slice_lower_bits(addr, m_addr_bits[4]); // Row
    }
};
